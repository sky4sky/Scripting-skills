...
**Scripting (iOS) mapping:** treat these as satisfied if the substitutes work.
- `web_search`: Brave Search MCP if connected; otherwise GitHub MCP search, `browser`, or `curl` against public pages. Do not stop solely because Brave is disconnected.
- `web_fetch`: `browser` tool or `curl` / HTTP. Prefer official docs and primary sources.
- Subagents: the Scripting `subagent` tool. If dispatch fails, run tasks sequentially in this session.
- Notes go under the current workspace (e.g. `research-notes/`), not a desktop path.
